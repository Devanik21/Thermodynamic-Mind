...
with tab_meta:
    ...
    if st.session_state.get('viewing_loaded_dna', False):
        ...
        # 🔍 DIAGNOSTIC: Permanent prominent debug info in Viewing Mode
        with st.expander("🔍 VIEWING MODE DATA AUDIT (Debug)", expanded=False):
            _meta = loaded.get('metacognition', {})
            st.write(f"**ZIP Meta:** {loaded.get('metadata', {})}")
            st.write(f"**L5 Preserved Concepts:** {len(_meta.get('level_5', {}).get('concept_points', []))}")
            st.write(f"**L9 Preserved Causal:** {len(_meta.get('level_9', {}).get('causal_data', []))}")
            st.write(f"**L10 Preserved Energy:** {len(_meta.get('level_10', {}).get('energies_10', []))}")
            st.write(f"**Raw L10 Keys:** {list(_meta.get('level_10', {}).keys())}")
        ...
        with st.expander("🧠 Level 5: Meta-Learning & Architecture", expanded=True):
            ...
            if st.session_state.get('show_charts', False):
                ...
                with c5_3:
                    ...
                    _concepts = []
                    if _viewing_dna:
                        # Priority 1: Use Cache Injection
                        if cache.get('concept_points'):
                            _concepts = [np.array(p) for p in cache['concept_points']]
                        # Priority 2: Use Raw DNA fallback
                        if not _concepts:
                            try:
                                _dna_cp = st.session_state.loaded_dna.get('metacognition', {}).get('level_5', {}).get('concept_points', [])
                                if _dna_cp: _concepts = [np.array(p) for p in _dna_cp]
                            except Exception: pass
                    elif all_agents:
                        # Live mode only
                        for a in all_agents[:50]:
                            if hasattr(a, 'last_concepts') and a.last_concepts is not None:
                                ...
                                if len(c_vec) >= 2: _concepts.append(c_vec[:2])
        ...
        with st.expander("⚛️ Level 9: Physics Discovery & Reality Hacking", expanded=True):
            ...
            if st.session_state.get('show_charts', False):
                ...
                with c9_4:
                    ...
                    _causal_data = []
                    if _viewing_dna:
                        if c9.get('causal_data'):
                             _causal_data = c9['causal_data']
                        else:
                             try:
                                 _dna_cd = st.session_state.loaded_dna.get('metacognition', {}).get('level_9', {}).get('causal_data', [])
                                 if _dna_cd: _causal_data = _dna_cd
                             except Exception: pass
                    elif all_agents:
                         sample = all_agents[0]
            ...
            if st.session_state.get('show_charts', False):
                ...
                with c10_3:
                    ...
                    if _viewing_dna:
                        ...
                        _x_dim = [a.energy for a in all_agents]
                        _y_dim = [a.age for a in all_agents]
                        _z_dim = [getattr(a, 'confidence', 0.5) for a in all_agents]
