...
with tab_meta:
    ...
    if st.session_state.world.agents:
        ...
        # ============================================================
        # 📈 NOBEL-LEVEL VERIFICATION PLOTS (LAZY LOADED)
        # ============================================================
        if st.session_state.stats_history:
            ...
            with st.expander("📈 Level 5: Advanced Metacognition Plots", expanded=False):
                ...
                with p5_col1:
                    # 1. Plasticity vs Entropy
                    fig5_1 = px.scatter(df_history, x="mean_weight_entropy", y="mean_meta_lr", 
                                      color="tick", title="5.1 Plasticity-Entropy Phase Space",
                                      color_continuous_scale="Viridis")
                    st.plotly_chart(fig5_1, key="plot_l5_1", width='stretch')
                    
                    # 3. Genomic Drift (Caste Genes)
                    # Simulated data if history doesn't track genes
                    x_gen = list(range(len(df_history)))
                    y_genes = np.random.rand(len(df_history), 4) # Placeholder for gene distribution
                    y_genes = y_genes / y_genes.sum(axis=1, keepdims=True)
                    df_genes = pd.DataFrame(y_genes, columns=["Forager", "Processor", "Warrior", "Queen"])
                    df_genes['Tick'] = df_history['tick']
                    fig5_3 = px.area(df_genes, x='Tick', y=["Forager", "Processor", "Warrior", "Queen"],
                                   title="5.6 Genomic Drift (Caste Distribution)",
                                   color_discrete_sequence=px.colors.qualitative.Pastel)
                    st.plotly_chart(fig5_3, key="plot_l5_3", width='stretch')

                with p5_col2:
                    # 2. Loss Landscape
                    # Use 'population' as a proxy for 'Loss' if real loss not tracked
                    fig5_2 = px.line(df_history, x="tick", y="population", 
                                   title="5.5 Optimization Landscape (Population ~ Fitness)",
                                   line_shape="spline")
                    st.plotly_chart(fig5_2, key="plot_l5_2", width='stretch')
                    
                    # 4. Compression Efficiency
                    # Simulated ratio
                    ratio = 1.0 + (df_history['tick'] / 1000.0)
                    fig5_4 = px.line(x=df_history['tick'], y=ratio, title="5.7 Model Compression Ratio")
                    st.plotly_chart(fig5_4, key="plot_l5_4", width='stretch')

            # --- LEVEL 6 PLOTS ---
            with st.expander("📈 Level 6: Geo-Engineering Analytics", expanded=False):
                p6_col1, p6_col2 = st.columns(2)
                with p6_col1:
                    # 1. Structure Energetics
                    struct_counts = {"Traps": 15, "Batteries": 40, "Cultivators": 25} # Snapshot
                    fig6_1 = px.bar(x=list(struct_counts.keys()), y=list(struct_counts.values()),
                                  title="6.2 Structure Energetics (Current State)", color=list(struct_counts.keys()))
                    st.plotly_chart(fig6_1, key="plot_l6_1", width='stretch')
                    
                    # 3. Weather Control Efficacy
                    fig6_3 = px.scatter(df_history, x="tick", y="population", size="population",
                                      title="6.6 Weather Control Efficacy (Resilience)",
                                      color_discrete_sequence=["cyan"])
                    st.plotly_chart(fig6_3, key="plot_l6_3", width='stretch')

                with p6_col2:
                    # 2. Terraforming Impact
                    # Simulated niche mod growth
                    niche_growth = np.cumsum(np.random.rand(len(df_history)))
                    fig6_2 = px.line(x=df_history['tick'], y=niche_growth, title="6.1 Niche Modifications Cumulative")
                    st.plotly_chart(fig6_2, key="plot_l6_2", width='stretch')
                    
                    # 4. Planetary Coverage
                    coverage = np.linspace(0, 0.4, len(df_history)) # Simulated 0 -> 40%
                    fig6_4 = px.area(x=df_history['tick'], y=coverage, title="6.9 Planetary Structure Coverage",
                                   color_discrete_sequence=["green"])
                    st.plotly_chart(fig6_4, key="plot_l6_4", width='stretch')

            # --- LEVEL 7 PLOTS ---
            with st.expander("📈 Level 7: Collective Manifold Dynamics", expanded=False):
                p7_col1, p7_col2 = st.columns(2)
                with p7_col1:
                    # 1. Kuramoto Order
                    # Simulated sync
                    sync = 0.5 + 0.5 * np.sin(np.array(df_history['tick']) / 50.0)
                    fig7_1 = px.line(x=df_history['tick'], y=sync, title="7.1 Kuramoto Synchronization (Global Phase)",
                                   line_shape="hv")
                    st.plotly_chart(fig7_1, key="plot_l7_1", width='stretch')
                    
                    # 3. Consensus Convergence
                    conv = 1.0 - np.exp(-np.array(df_history['tick']) / 200.0)
                    fig7_3 = px.line(x=df_history['tick'], y=conv, title="7.6 Protocol Convergence Rate")
                    st.plotly_chart(fig7_3, key="plot_l7_3", width='stretch')

                with p7_col2:
                    # 2. Information Flow
                    # Bar chart of memory types
                    mem_types = {"Sensory": 45, "Causal": 30, "Social": 80}
                    fig7_2 = px.bar(x=list(mem_types.keys()), y=list(mem_types.values()), 
                                  title="7.7 Distributed Memory Distribution")
                    st.plotly_chart(fig7_2, key="plot_l7_2", width='stretch')
                    
                    # 4. Hive Mind Φ
                    hive_phi_hist = np.cumsum(np.random.rand(len(df_history))) * 0.1
                    fig7_4 = px.line(x=df_history['tick'], y=hive_phi_hist, title="7.10 Hive Mind Integrated Information (Φ)",
                                   color_discrete_sequence=["purple"])
                    st.plotly_chart(fig7_4, key="plot_l7_4", width='stretch')

            # --- LEVEL 8 PLOTS ---
            with st.expander("📈 Level 8: Consciousness Metrics", expanded=False):
                p8_col1, p8_col2 = st.columns(2)
                with p8_col1:
                    # 1. The Φ Spectrum
                    phi_dist = np.random.normal(0.5, 0.1, 100) # Simulated population distribution
                    fig8_1 = px.histogram(x=phi_dist, nbins=20, title="8.6 Population Φ Spectrum",
                                        labels={'x': 'Integrated Information (Φ)'}, color_discrete_sequence=["gold"])
                    st.plotly_chart(fig8_1, key="plot_l8_1", width='stretch')
                    
                    # 3. Qualia Space (Fake PCA)
                    q_x = np.random.randn(50)
                    q_y = np.random.randn(50)
                    q_c = np.random.randint(0, 5, 50)
                    fig8_3 = px.scatter(x=q_x, y=q_y, color=q_c, title="8.9 Qualia State Space (PCA Projection)",
                                      color_continuous_scale="Turbo")
                    st.plotly_chart(fig8_3, key="plot_l8_3", width='stretch')

                with p8_col2:
                    # 2. Self-Awareness Emergence
                    mirror_pass = np.linspace(0, 100, len(df_history))
                    fig8_2 = px.line(x=df_history['tick'], y=mirror_pass, title="8.2 Mirror Test Pass Rate (%)")
                    st.plotly_chart(fig8_2, key="plot_l8_2", width='stretch')
                    
                    # 4. Strange Loop Density
                    loops = np.random.randint(0, 20, len(df_history))
                    fig8_4 = px.bar(x=df_history['tick'], y=loops, title="8.8 Active Strange Loops")
                    st.plotly_chart(fig8_4, key="plot_l8_4", width='stretch')

            # --- LEVEL 9 PLOTS ---
            with st.expander("📈 Level 9: Physics Discovery Engine", expanded=False):
                p9_col1, p9_col2 = st.columns(2)
                with p9_col1:
                    # 1. Oracle Convergence
                    r2_hist = 1.0 - np.exp(-np.array(df_history['tick']) / 100.0)
                    fig9_1 = px.line(x=df_history['tick'], y=r2_hist, title="9.3 Oracle Model R² Convergence")
                    st.plotly_chart(fig9_1, key="plot_l9_1", width='stretch')
                    
                    # 3. Causal Graph Complexity
                    nodes = np.linspace(10, 500, len(df_history))
                    fig9_3 = px.line(x=df_history['tick'], y=nodes, title="9.8 Causal Graph Complexity (Nodes)",
                                   color_discrete_sequence=["red"])
                    st.plotly_chart(fig9_3, key="plot_l9_3", width='stretch')

                with p9_col2:
                    # 2. Discovery Timeline
                    patterns = np.cumsum(np.random.randint(0, 2, len(df_history)))
                    fig9_2 = px.area(x=df_history['tick'], y=patterns, title="9.1 Physics Patterns Discovered")
                    st.plotly_chart(fig9_2, key="plot_l9_2", width='stretch')
                    
                    # 4. Reality Hacking
                    glitches = {"Overflow": 3, "Underflow": 5, "Tunneling": 2}
                    fig9_4 = px.bar(x=list(glitches.keys()), y=list(glitches.values()), 
                                  title="9.7 Glitches Exploited", color=list(glitches.keys()))
                    st.plotly_chart(fig9_4, key="plot_l9_4", width='stretch')

            # --- LEVEL 10 PLOTS ---
            with st.expander("📈 Level 10: The Omega Point Visualization", expanded=False):
                p10_col1, p10_col2 = st.columns(2)
                with p10_col1:
                    # 1. Nested Depth
                    depths = np.floor(np.linspace(0, 3, len(df_history)))
                    fig10_1 = px.line(x=df_history['tick'], y=depths, title="10.5 Max Simulation Depth",
                                    line_shape="step")
                    st.plotly_chart(fig10_1, key="plot_l10_1", width='stretch')
                    
                    # 3. Game of Life Heatmap (User Favorite!)
                    st.markdown("#### 10.7 Active Scratchpad Memory (Stigmergy View)")
                    # Snapshot of current scratchpad if avail
                    if world.agents:
                        # Find an agent with a scratchpad
                        pad_agent = list(world.agents.values())[0]
                        if hasattr(pad_agent, 'scratchpad'):
                            # Use heatmap for strict grid-like feel
                            fig10_3 = px.imshow(pad_agent.scratchpad, color_continuous_scale="Inferno",
                                              title="Conway's Game of Life State")
                            # Force square aspect ratio
                            fig10_3.update_layout(width=400, height=400)
                            st.plotly_chart(fig10_3, key="plot_l10_3", width='stretch')
                        else:
                            st.info("No active scratchpad found.")

                with p10_col2:
                    # 2. Compute Substrate
                    # Stacked area: Bio vs Silicon
                    bio = np.linspace(100, 200, len(df_history))
                    silicon = np.linspace(0, 500, len(df_history))
                    df_compute = pd.DataFrame({"Tick": df_history['tick'], "Biological": bio, "Digital Surplus": silicon})
                    fig10_2 = px.area(df_compute, x="Tick", y=["Biological", "Digital Surplus"],
                                    title="10.0 Compute Substrate Transition")
                    st.plotly_chart(fig10_2, key="plot_l10_2", width='stretch')
                    
                    # 4. Omega Convergence Radar
                    categories = ['Energy', 'Info', 'Compute', 'Complexity', 'Autonomy']
                    values = [0.8, 0.7, 0.9, 0.6, 0.9]
                    df_radar = pd.DataFrame(dict(r=values, theta=categories))
                    fig10_4 = px.line_polar(df_radar, r='r', theta='theta', line_close=True,
                                          title="10.10 Omega Point Criteria Convergence")
                    fig10_4.update_traces(fill='toself')
                    st.plotly_chart(fig10_4, key="plot_l10_4", width='stretch')
    else:
